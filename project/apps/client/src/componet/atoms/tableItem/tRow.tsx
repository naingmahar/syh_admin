        ...
        <tr>
            ...
            </th>
            {
                props.types.map((row,index) => {
                    ...
                    if (row.type == "details") return <td key={index} className={row.className}><button className="btn-link" onClick={()=>{
                        navigate(row.detailsLink?.(props.data) || "/")
                    }} title="Details"> Deatils </button></td>
                    if(row.type == "delete") return <td key={index} className={row.className}>
                        <button onClick={()=>row.deleteAction ? row.deleteAction(props.data) : null}  
                            className="btn btn-error btn-sm text-white"> Delete </button></td>
                    if (row.type == "link") return <td key={index} className={row.className}><a className="link-primary" href={rowData} target="_blank">{"Link"}</a></td>
                    if (row.type == "image") return <ImageColumn key={index} className={row.className} img={rowData} alt={row.alt} />
                    if (row.type == "changeStatus") return <StatusChangeDropDown key={index} label={rowData} onChange={row.action?.onChange} />
